...
namespace bero_System.projectLevelSellForms
{
    public partial class FrmEditprojectLevelSell : RadForm
    {
        ...
        private void FillCombo()
        {
            ...
            var q1 = projectLevelCommand.GetAll().Where(p=>p.ProjectProfileID == ProjectID) ;
            this.Invoke((MethodInvoker)delegate
            {
        ...
        private void SaveBtn_Click(object sender, EventArgs e)
        {
            ...
            int Acct =  projectLevelCommand.GetByID(int.Parse(projectLevelComboBox.SelectedValue.ToString())).AccountID.Value;
            double balance = AccountDailyCommand.GetBalanceByAccountID(Acct);
             

            #endregion
            int id = TargetSelTable.ID;

            projectLevelSell lvl = new projectLevelSell();
            lvl = TargetSelTable;
            this.TargetSelTable = new projectLevelSell();
            TargetSelTable.ID = id;
